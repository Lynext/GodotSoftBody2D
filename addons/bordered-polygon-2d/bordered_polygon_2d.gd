...
func smooth_shape_points(shape_points, max_angle):
	...
	for i in range(SMOOTH_MAX_PASSES): # max passes
		...
		for j in range(shape_points.size()):
			...
			var a = shape_points[(j + current_shape_size - 1) % current_shape_size]
			var b = shape_points[j]
			var c = shape_points[(j + 1) % current_shape_size]
			...
			if triad_angle < max_angle:
				...
				if angle_gain > SMOOTH_MIN_ANGLE_GAIN:
					...
					point_to_smooth.append([j, smoothed_points])
